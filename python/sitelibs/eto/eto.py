...
def ETo(Rn, t, ws, es, ea, delta_es, psy, crop, shf=0.0):
    """
    ...
    #Crop Constants grass vs shrubs
    grass = [900, 0.34]
    shrub = [1200, 0.38]
    Cn = grass[0]
    Cd = grass[1]
    ...
    a1 = 0.408 * (Rn - shf) * delta_es / (delta_es + (psy * (1 + Cd * ws)))
    a2 = Cn * ws / t * (es - ea) * psy / (delta_es + (psy * (1 + Cd * ws)))
    ETo = a1 + a2
    return ETo
